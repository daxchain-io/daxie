...
type Check struct {
    ...
    Acked     bool     // explicit unlimited acknowledgment — CLI --unlimited --yes; MCP acknowledgeUnlimited:true
...
the `--unlimited --yes` / `acknowledgeUnlimited` ceremony, and `allow_unlimited:false`
...
`--unlimited --yes` and the MCP tool's `acknowledgeUnlimited: true` both set
...
> unlimited-approval ack field is **`acknowledgeUnlimited`** (mapping to `Check.Acked`).
...
than a struct field is **`acknowledgeUnlimited`** — and the Architecture put it directly
...
```json
{
  ...
  "properties": {
    ...
    "unlimited": {"type": "boolean", "description": "Grant an unbounded (max-uint256) allowance. Requires acknowledgeUnlimited:true."},
    "acknowledgeUnlimited": {"type": "boolean", "description": "Required when the approval is UNLIMITED. Grants the spender an unbounded allowance over the token. Omit unless that is the explicit intent."},
...
is the one safety ack and it is in the schema (`acknowledgeUnlimited`, never frontend-set,
...
| Unlimited-approval/permit ceremony | `policy.Evaluate` (`Acked`) | `acknowledgeUnlimited` field; absent ⇒ `policy.denied.unlimited_unacked` | ack is a **schema field**, not a flag |
...
| `token approve` the world | Approvals are spend-equivalents: spender must pass the allowlist; unlimited requires `--unlimited --yes` / `acknowledgeUnlimited`; `allow_unlimited:false` is an operator hard-deny. Residual: a hijacked agent controls its own `--yes`, so absent a per-token `allow_unlimited:false` it *can* sign an unlimited approval to an allowlist-passing spender (documented hardening: one rule per held token) | exit 3 |

...
// normalizeEOL collapses CRLF to LF so the byte-for-byte golden comparison is
// stable across platforms. On Windows the golden fixture can be checked out with
// CRLF (git autocrlf) even though we ship a .gitattributes pinning eol=lf — this
// belt-and-suspenders normalization keeps a stale local checkout passing without
// weakening the schema-drift assertion (we only erase line-ending noise, never
// content).
...
func TestToolSchemasGolden(t *testing.T) {
	...
	// Normalize line endings on BOTH sides before comparing: the generated bytes
	// always use "\n", but the on-disk golden may be CRLF on a Windows checkout.
	// This compares schema content, not line-ending encoding (see normalizeEOL).
